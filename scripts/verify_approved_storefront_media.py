...
MIN_INSET = 96
COHORT_WIDTH_SPREAD = 0.18
COHORT_HEIGHT_SPREAD = 0.10
COHORT_AREA_SCALE_SPREAD = 0.08
COHORT_AXES = {
    "side": {"width"},
    "three-quarter": {"width"},
    "front": {"width"},
    # Complete rear pairs legitimately vary in width when the source has a
    # different heel spacing. Compare visual scale by geometric area so a
    # fully visible wide pair is not forced into a crop or distortion.
    "rear": {"area"},
    "sole": {"height"},
    "detail": {"width", "height"},
}
...
OCCUPANCY_LIMITS = {
    "footwear": {
        ...
        "three-quarter": ((0.56, 0.86), (0.36, 0.68)),
...
def verify_manifest(manifest_path: Path, asset_root: Path) -> None:
    ...
    for (product_type, angle), values in cohorts.items():
        ...
        if "area" in COHORT_AXES[base_angle]:
            scales = [math.sqrt(width * height) for width, height in zip(widths, heights, strict=True)]
            if max(scales) - min(scales) > COHORT_AREA_SCALE_SPREAD:
                errors.append(
                    f"{product_type}/{angle}: cohort area-scale spread {max(scales) - min(scales):.3f} "
                    f"exceeds {COHORT_AREA_SCALE_SPREAD:.3f}"
                )
    if errors:
