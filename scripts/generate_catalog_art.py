#!/usr/bin/env python3
"""Generate the standalone catalog's original demo illustrations.

The output is deliberately illustrative: these are not manufacturer photos and
must not be presented as exact product photography.  Runtime and production
builds use only the committed WebP files. This optional helper writes to
site/generated/catalog-art and cannot overwrite the exact catalog by default.
"""
...
def create_background(index: int, accent: str) -> Image.Image:
    image = Image.new("RGB", (WIDTH * SCALE, HEIGHT * SCALE), "#f7f8fa")
    ...
    grid = (219, 224, 231)
    for x in range(sc(80), sc(WIDTH), sc(160)):
        draw.line((x, 0, x, sc(HEIGHT)), fill=grid, width=max(1, SCALE))
    for y in range(sc(72), sc(HEIGHT), sc(144)):
        draw.line((0, y, sc(WIDTH), y), fill=grid, width=max(1, SCALE))

    # A shifted halo makes every product portrait recognisably different while
    # preserving the quiet white catalog system.
    halo = Image.new("RGBA", image.size, (0, 0, 0, 0))
    halo_draw = ImageDraw.Draw(halo)
    offset = ((index * 47) % 180) - 90
    halo_draw.ellipse(
        box((350 + offset, 118, 1080 + offset, 848)),
        fill=accent + "26",
        outline=accent + "54",
        width=sc(2),
    )
    halo = halo.filter(ImageFilter.GaussianBlur(sc(7)))
    return Image.alpha_composite(image.convert("RGBA"), halo)


def shadow_layer() -> Image.Image:
    shadow = Image.new("RGBA", (WIDTH * SCALE, HEIGHT * SCALE), (0, 0, 0, 0))
    draw = ImageDraw.Draw(shadow)
    draw.ellipse(box((290, 742, 1120, 892)), fill=(10, 20, 35, 56))
    return shadow.filter(ImageFilter.GaussianBlur(sc(30)))
...
def draw_shoe(image: Image.Image, product: ProductArt, index: int) -> None:
    ...
    lift = 84 if high else 0
    rear = 365 if high else 425
    toe = 1115 if chunky else 1080
    sole_top = 720 if chunky else 748
    sole_bottom = 842 if chunky else 825
    ...
    draw.polygon(points(upper), fill=product.primary, outline="#22272d", width=sc(5))
    draw.rounded_rectangle(
        box((338, sole_top, toe + 22, sole_bottom)),
        radius=sc(34 if chunky else 26),
        fill=product.secondary,
        outline="#252a30",
        width=sc(5),
    )
    draw.line(
        points(((382, sole_bottom - 17), (1030, sole_bottom - 17))),
        fill="#30353a",
        width=sc(7),
    )
    if high:
        draw.rounded_rectangle(
            box((402, 342 - lift, 610, 675)),
            radius=sc(42),
    ...
    # Layered panel construction resembles a real product portrait without
    # reproducing any protected brand mark.
    draw.polygon(
        points(((470, 650), (555, 485 - lift // 2), (690, 555), (660, 696))),
        fill=product.secondary,
        outline="#33383d",
        width=sc(4),
    )
    draw.polygon(
        points(((685, 570), (808, 634), (990, 688), (920, 716), (715, 685))),
        fill=product.accent,
        outline="#33383d",
        width=sc(4),
    )
    if classic:
        draw.polygon(
            points(((490, 652), (620, 505), (760, 625), (665, 699))),
            fill=product.secondary,
            outline="#33383d",
            width=sc(4),
        )
        ...
        for offset in (0, 44, 88):
            draw.line(
                points(((560 + offset, 536), (738 + offset, 676))),
                fill=product.accent,
                width=sc(18 if product.silhouette == "runner" else 13),
            )
    for step in range(5):
        y = 530 + step * 35 - lift // 3
        draw.line(
            points(((565 + step * 9, y), (750 + step * 12, y + 65))),
            fill="#f6f7f8",
            width=sc(7),
        )
    draw.ellipse(box((505, 645, 550, 690)), fill=product.accent, outline="#262b31", width=sc(4))
    draw.ellipse(box((930, 692, 985, 745)), fill=product.accent, outline="#262b31", width=sc(4))
    if trail:
...
def write_manifests(records: list[dict[str, object]]) -> None:
    payload = {
        ...
        "disclaimer": (
            "Original cold-studio geometric illustrations generated inside this project. "
    ...
    lines = [
        ...
        "",
        "All 60 WebP files are original cold-studio geometric illustrations generated by "
