...
class ApprovedStorefrontMediaTests(unittest.TestCase):
    ...
    def write_frame(
        ...
        *,
        ...
        fill = (
            (30 + number + color_offset, 50, 70, 255)
            if transparent
            else (30 + number + color_offset, 50, 70)
        )
    ...
    def frames(self, prefix: str, *, overrides=None, color_offset: int = 0):
        boxes = [
            (240, 280, 1360, 880),
            (240, 280, 1360, 880),
            (240, 280, 1360, 880),
    ...
    def test_reports_inconsistent_scale_inside_a_type_and_angle_cohort(self):
        ...
        second = self.sku(
            "SECOND",
            self.frames("second", overrides={0: (480, 350, 1120, 810)}),
        )
        with self.assertRaisesRegex(
            gate.MediaQaError,
    ...
    def test_accepts_rear_pairs_with_matching_area_scale_and_different_aspect_ratios(self):
        first = self.sku("FIRST", self.frames("first"))
        second = self.sku(
            "SECOND",
            self.frames(
                "second",
                overrides={3: (320, 248, 1280, 952)},
                color_offset=20,
            ),
        )
